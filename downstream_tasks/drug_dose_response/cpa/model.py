...
class CPA(torch.nn.Module):
    ...
    def predict(
        ...
        if emb_file == None:
            raise ValueError("You must got embedding, please use get_emb.py to generate them or provide related .npy file first.")
        else:
            gene_embedding = torch.from_numpy(np.load(emb_file))
        ...
        m = nn.Linear(gene_embedding.size(1), 128)
        gene_embedding = m(gene_embedding).cuda()

        #print(gene_embedding.size())
        latent_treated = torch.cat([latent_treated, gene_embedding])
        # latent_treated = latent_treated + gene_embedding
