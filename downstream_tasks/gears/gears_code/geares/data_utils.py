...
# 通过协变量（covariate）的不同水平来计算基因在不同实验组之间的表达差异。
'''
协变量(covariate)是指在单细胞转录组学研究中,与表达矩阵中的基因表达量一起记录的其他细胞属性或特征，例如细胞类型、实验批次、处理条件、年龄、性别等
这里这样看的话,其实是有点类似于我们这里的condition?
不过这里看了一下，好像是cell_type，有些奇怪这样的话。
'''
# 具体来说，它将adata数据集中的细胞按照协变量分类，然后对每个协变量水平内的细胞进行差异表达基因（DEG）分析，得到每个实验组相对于参考对照组的差异表达基因列表。
# 最后，将这些基因列表存储在adata.uns[key_added]中，以便后续的分析
def rank_genes_groups_by_cov(
    ...
    for cov_cat in cov_categories:
        ...
        # 在adata中用于分组的观测元数据列（即groupby参数指定的列）中，控制对照组的名称
        control_group_cov = '_'.join([cov_cat, control_group])

        #subset adata to cells belonging to a covariate category
        # 将adata数据集中的细胞按照协变量（covariate）分类，并从中选择属于某个协变量水平的细胞子集进行分析
        adata_cov = adata[adata.obs[covariate]==cov_cat]

        #compute DEGs
        # 通过协变量分类后，针对每个协变量水平内的实验组，计算差异表达基因（DEG）列表。
        # 具体来说，这里使用了Scanpy（一个流行的单细胞RNA测序分析工具）的rank_genes_groups函数，对每个协变量水平内的实验组进行差异表达分析。
        # rank_genes_groups函数是Scanpy中用于计算每个实验组相对于参考对照组的差异表达基因的函数
...
# 用于从单细胞转录组数据中获取与基因表达量相关的信息，包括平均表达量、非零基因和非失活基因等
def get_dropout_non_zero_genes(adata):
    
    # calculate mean expression for each condition
    # 首先，根据单细胞转录组数据中的条件信息，计算每个条件下所有基因的平均表达量，并将结果存储在condition2mean_expression字典中。
    ...
    # 将condition2mean_expression字典转换为pert_list和mean_expression两个数组，分别表示每个条件的名称和平均表达量矩阵。
    ...
    # 根据平均表达量的特征，筛选出非零基因和非失活基因的索引，分别存储在non_zeros_gene_idx和non_dropout_gene_idx字典中
...
class DataSplitter():
    ...
    这段描述是关于一个数据分割处理的类的说明。该类可以生成新的数据分割，并将其分配为数据文件的新属性
    """
