...
class GeneSimNetwork():
    # edge_list是一个包含基因相似性边列表的DataFrame对象
    # gene_list是一个包含所有基因名称的列表
    # node_map是一个将基因名称映射到节点索引的字典。
    def __init__(self, edge_list, gene_list, node_map):
        # 使用nx.from_pandas_edgelist()函数将edge_list转换为一个有向图，其中每个基因名称对应一个节点，每条边表示两个节点之间的相似性。
        # 然后，代码检查gene_list中的每个基因是否在图中存在，如果不存在，则将其添加到图中
        ...
        for n in self.gene_list:
            if n not in self.G.nodes():
                self.G.add_node(n)
        # 将节点名称映射到索引，并将所有边的源节点和目标节点的索引存储在edge_index_列表中
    ...
    if network_type == 'co-express':
        ...
        if default_pert_graph:
            ...
            tar_data_download_wrapper(server_path, 
                                     ...
                                     data_path)
            # 基于GO注释数据集，计算基因之间的相似性，得到GO注释的共表达网络。
...
# 这里K的定义就是找出来每一个基因与之最相关的k个基因。
# threshold是能接受的相关度的阈值
# 数据集的路径和名称
# split表示训练集和测试集划分比例
# seed是随机数种子
def get_coexpression_network_from_train(adata, threshold, k, data_path,
    ...
    if os.path.exists(fname):
        ...
        out[np.isnan(out)] = 0
        out = np.abs(out)
        # 返回了一个按升序排列的数组索引矩阵，所以取最后的相关性最强的k个基因
        out_sort_idx = np.argsort(out)[:, -(k + 1):]
        # 折回去，获得一个基因与其最相关的k个基因之间的共表达系数矩阵。
...
def create_cell_graph_for_prediction(X, pert_idx, pert_gene,emb):

    if pert_idx is None:
        pert_idx = [-1]
    # print(X.shape,pert_idx,pert_gene)
    return Data(x=torch.Tensor(X).T, pert_idx = pert_idx, pert=pert_gene,emb=emb)
    
# 从对照组数据中随机采样num_samples个单个细胞，并分别将它们转换为Data对象，用于模型的预测。
# 具体来说，该函数会从ctrl_adata中随机选择num_samples个单个细胞，并将它们的基因表达量矩阵转换为Data对象。
# 对于每个Data对象，会使用create_cell_graph_for_prediction函数将其转换为Data对象，并将所有Data对象存储在一个名为cell_graphs的列表中。
# 最后，函数将cell_graphs列表返回。
def create_cell_graph_dataset_for_prediction(pert_gene, ctrl_adata, gene_names,
    ...
    rand_list=np.random.randint(0, len(ctrl_adata), num_samples)
    # print(rand_list)
    # Xs = ctrl_adata[rand_list, :].X.toarray()
    Xs = ctrl_adata[rand_list, :]
    # Create cell graphs
    cell_graphs = [create_cell_graph_for_prediction(Xs[i].X.toarray(), pert_idx, pert_gene,(Xs.obs['flag'][i])).to(device) for i in range(len(Xs))]
    return cell_graphs
...
# 接受一个扰动列表perts作为输入，并返回这些扰动涉及的基因列表
