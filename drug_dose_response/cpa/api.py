...
class API:
    ...
    def get_drug_embeddings(self, dose=1.0, return_anndata=True):
        ...
        # print('emb_perts:', emb_perts.shape)
        if return_anndata:
    ...
    def predict(
        ...
        for i in range(len(df)):
            ...
            gene_reconstructions = (
                self.model.predict(genes, drugs, covars, emb_file).cpu().clone().detach().numpy()
            )
    ...
    def get_response(
        ...
        # print('genes_controls:', genes_control.size())
        j = 0
    ...
    def evaluate_r2(self, dataset, genes_control, emb_file, adata_random=None):
        """
